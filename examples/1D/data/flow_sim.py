...
@jit(nopython=True, fastmath=True)
def compute_absolute_mobility(swat, pwat, kwat, soil, poil, koil, vr):
...
@jit(nopython=True, fastmath=True)
def compute_velocity(pmin, pmax, sdata, pwat, kwat, poil, koil, vr):
    lmob = compute_mobility_integral(sdata, pwat, kwat, poil, koil, vr)
    return (pmax - pmin) / lmob


@jit(nopython=True, fastmath=True)
def update_saturations(s, dt, dx, poro, pmin, pmax, pwat, kwat, poil, koil, vr):
...
@jit(nopython=True, fastmath=True)
def compute_saturation_data(nx, nt, tfinal, poro, pmin, pmax, pwat, kwat, poil, koil, vr):
    dt = tfinal / nt
    dx = 1.0 / nx
    # init saturations
    s0 = np.zeros(nx)
...
def compute_solution(
    ...
    s = compute_saturation_data(nx, nt, t, poro, pmin, pmax, pwat, kwat, poil, koil, vr)
...
compute_solution_vec = np.vectorize(compute_solution)
